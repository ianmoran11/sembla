...
variable "ssh_host_private_key" {
  description = "Optional ephemeral ED25519 SSH *host* private key, pre-seeded so the host-key fingerprint is known before first boot and trust-on-first-use never occurs. Supply only through TF_VAR_ssh_host_private_key, generated by prepare-host-key.sh. Empty keeps the image's own host key and the VNC-console verification path."
  type        = string
  sensitive   = true
  default     = ""

  validation {
    condition = var.ssh_host_private_key == "" || can(regex(
      "^-----BEGIN OPENSSH PRIVATE KEY-----\n[A-Za-z0-9+/=\n]+-----END OPENSSH PRIVATE KEY-----\n?$",
      var.ssh_host_private_key,
    ))
    error_message = "ssh_host_private_key must be empty or one OpenSSH-format private key supplied through TF_VAR_ssh_host_private_key."
  }
}

variable "evidence_deploy_key" {
  description = "Optional ED25519 *deploy* private key whose public half is registered on the sembla GitHub repository with write access. When set, the remote payload pushes its evidence directory to an evidence/* branch so collection no longer needs a live SSH session at the end of a long run. Supply only through TF_VAR_evidence_deploy_key, generated by prepare-deploy-key.sh. Empty disables the push and keeps SSH collection."
  type        = string
  sensitive   = true
  default     = ""

  validation {
    condition = var.evidence_deploy_key == "" || can(regex(
      "^-----BEGIN OPENSSH PRIVATE KEY-----\n[A-Za-z0-9+/=\n]+-----END OPENSSH PRIVATE KEY-----\n?$",
      var.evidence_deploy_key,
    ))
    error_message = "evidence_deploy_key must be empty or one OpenSSH-format private key supplied through TF_VAR_evidence_deploy_key."
  }
}
