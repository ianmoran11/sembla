...
| Complete local | `./scripts/check.sh` | Requires Cargo, Git, and Python 3. Runs documentation and architecture checks (including their temporary-fixture tests), regenerates and verifies the pinned frontend compatibility surface without Lean, and runs the full Rust contract; a missing tool is an error, never a skip. |
