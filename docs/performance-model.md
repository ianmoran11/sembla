...
| 12 | [`prds-sweep-throughput/0004`](prds-sweep-throughput/0004-run-cuda-draws-concurrently.md) run CUDA draws concurrently | local only | **landed and locally approved** in `71c76fd`: explicit bounded `--draw-workers` interface; GPU criteria remain pending per §M3 |
