...
> This file does not say whether it supersedes or amends the repository's `docs/archive/roadmaps/version-roadmap-2026-07-25.md`, which in turn names `DESIGN.md` as the design authority. Add an adoption date/status, map these stages to the existing milestones, and state how conflicts with `DESIGN.md` and `DECISIONS.md` are resolved; an unresolved conflict should keep the affected PRD from becoming ready.
>
> **Resolution (2026-07-24):** addressed by §0 below.

## 0. Authority, adoption, and in-flight work

- **Adoption.** *Discharged 2026-07-25:* this document is committed as `docs/archive/roadmaps/forward-roadmap-2026-07-25.md` with a dated amendment note in `docs/archive/roadmaps/version-roadmap-2026-07-25.md`. It is now a roadmap of record alongside `docs/archive/roadmaps/version-roadmap-2026-07-25.md`, which retains the version milestones; where they conflict, `DESIGN.md` and `DECISIONS.md` win over both, and the conflict is resolved by explicit amendment rather than by precedence between the two roadmaps.
...
- **Milestone mapping.** Stages 1–2 build the empirical baseline that `docs/archive/roadmaps/version-roadmap-2026-07-25.md` v0.5 presupposes ("the milestone consumes populations, it does not manufacture them"); Track J *is* the v0.5 courts/queueing theme, executed under an evidence gate; C.0005/C.0006 are the v1.0 consolidation items; C.0002 continues the v0.2 differential-testing obligation. The Lean proof track remains as `docs/archive/roadmaps/version-roadmap-2026-07-25.md` defines it — parallel, opportunistic, never on the critical path — and is exempt from the serial PRD rule below because it is never `Active` in the runner.
...
**Scale note (superseded 2026-07-25 by `DECISIONS.md` §L1):** the earlier note treated CUDA as the fast path for national-scale sweeps and grouped observations as the blocker. That performance assumption is superseded. Grouped observations remain CPU-only under §K6/§K9, but §L1's serial validation passes make the no-grouped driver model itself unusable on CUDA until `docs/prds-cuda-validation-parallelism/` lands or is explicitly closed. Stage 1 retains the observation split below as a reporting contract, not as a performance strategy:

- **Conditioning data `x` is scalar summaries.** Sweeps, calibration, and CRN contrasts run on declared scalar summaries only — the thin `(θ, x)` export §G5 already defines — and define the CUDA-targeted path if the §L1 blocker clears. This is a restriction on what a *sweep* reports, not on what the model means, and it does not assert that current CUDA is performant.
...
- **The §K9 CUDA-grouped deferral is decided, not left open.** Grouped observations remain CPU-only under `DECISIONS.md` §L5. This validation-parallelism track admits the no-grouped model only and does not add grouped CUDA support; that remains a separate deferred construct.
...
> [!danger] Superseding amendment (2026-07-25, measured) — `DECISIONS.md` §L1 identifies validation as the blocker
> The reporting split assumed CUDA was the fast path for scalar-summary sweeps and grouped observations were the only thing keeping the driver model off it. The checked-in diagnostic baseline says otherwise. The CUDA arm records commit `f81fef9d90ad`; the CPU arm records `8857cb839220`. No `crates/**`, Cargo manifest/lockfile, or fixture files changed between them, but the pair does not establish an identical binary or state artifact and does not satisfy the §L4 gate:
>
...
>
> **The GPU is 12.3× slower than the CPU beside it.** `DECISIONS.md` §L1 records the cause: four generated kernels (`sembla_validate_claims`, `_transition`, `_effects`, `_outputs`) validate every row on a *single* GPU thread, per claim and per fallible expression, per tick. The simulation kernels are properly parallel; the validation around them is serial. The loops are emitted only for models that contest a resource or dereference a `Ref`, which is why SIR reaches ~1,380 ticks/sec on the same hardware and why **every construct this roadmap plans next — vacant-slot claiming, generation-safe references, household links, identity-preserving migration — lands on the same path.**
>
> **Consequences for this document.** The former CUDA-fast/grouped-only assumption is superseded by `DECISIONS.md` §L1. The observation split survives as a *reporting* contract but not as a performance strategy: there is currently no usable CUDA fast path for this driver model, so a national-scale calibration sweep is not merely inconvenient, it is infeasible on either backend until this track lands or is explicitly closed. E.0001's geography bound is unaffected (it was released by the memory measurement, not the GPU one). C.0002's corpus scope is unaffected. What changes is that **§0's Preflight gains a prerequisite**: `docs/prds-cuda-validation-parallelism/` either lands or is explicitly closed before any stage depends on GPU throughput.
>
> **Measured outcome (2026-07-26).** The fix implied above did not work, and the cause above was wrong. `prds-cuda-validation-parallelism` PRD 0002 parallelised the four validation kernels; they now cost 0.0% of GPU time and the run was unchanged — CUDA 5647.9s against CPU 434.6s at 10M, 13.0× slower. Kernel profiling then identified the real cost: `sembla_resolve_conflicts` is **quadratic** in rows (fitted exponent 1.96), reaching 77.9% of GPU time at 5M and ~176 of the 235 s/tick at 10M. It performs a nested scan over the resource table inside a per-row kernel, where `DESIGN.md` §4.2's **segmented argmin** — which the CPU oracle already uses — is the intended primitive. See `DECISIONS.md` §L6 and `docs/evidence/cuda-profile-20260726/`.
>
> The Preflight prerequisite therefore still stands and is now better specified: it is discharged by PRD 0006 (segmented argmin) landing or being closed, not by 0002.
