...
pub struct GeneratedCuda {
    ...
    pub(crate) transition_validation: Vec<bool>,
    pub(crate) exclusive_effect_writes: Vec<bool>,
...
impl<'a> Generator<'a> {
    ...
    fn emit(self) -> Result<GeneratedCuda, CudaError> {
        ...
        let (transition_kernels, transition_validation) = self.emit_transition_kernels(&mut out)?;
        ...
        Ok(GeneratedCuda {
            ...
            exclusive_effect_writes: exclusive_effect_writes(self.model),
    ...
    fn emit_transition_kernels(
        &self,
        out: &mut String,
    ) -> Result<(Vec<String>, Vec<bool>), CudaError> {
        ...
        for validated in self.model.transitions() {
            ...
            validation.push(out.len() != validation_start);
            out.push_str("    return;\n  }\n");
        }
        out.push_str("}\n");
    ...
    fn emit_validation_status_kernels(&self, out: &mut String) {
        ...
        // flags that winner reduction repopulates each tick during execution.
...
// SetAttr writes only the executing row. A destination mentioned once in the
// model cannot collide with another candidate or an earlier effect. Expression
// and range errors have already been committed by ordered effect validation.
fn exclusive_effect_writes(model: &ValidatedModel) -> Vec<bool> {
    let mut counts = std::collections::BTreeMap::new();
    for rule in model.transitions() {
        for Effect::SetAttr { attr, .. } in
            &model.model().boxes[rule.box_index].transitions[rule.transition_index].effects
        {
            *counts
                .entry((rule.box_index, rule.table_index, attr.as_str()))
                .or_insert(0_usize) += 1;
        }
    }
    model
        ...
        .iter()
        .map(|rule| {
            model.model().boxes[rule.box_index].transitions[rule.transition_index]
                .effects
                .iter()
                .all(|Effect::SetAttr { attr, .. }| {
                    counts[&(rule.box_index, rule.table_index, attr.as_str())] == 1
                })
        })
        .collect()
}
...
/// Builds backend execution kernels while preserving the public reference
/// generator and its frozen source fixture. The returned hash covers exactly
/// the optimized source compiled by NVRTC, including fused-slot rewriting.
#[cfg(any(feature = "cuda", test))]
pub(crate) fn generate_execution(
    model: &ValidatedModel,
    fused: bool,
) -> Result<GeneratedCuda, CudaError> {
    let mut generator = Generator::new(model)?;
    generator.execution = true;
    let mut generated = generator.emit()?;
    if fused {
        generated.source = fused_batch_source(&generated.source)?;
        generated.source_sha256 = hex(&Sha256::digest(generated.source.as_bytes()));
    }
    Ok(generated)
}

/// Generates the deterministic reference translation unit for a validated model.
