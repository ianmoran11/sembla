...
pub(crate) fn sweep_file_result_with_runtime<R: SweepRuntime>(
    ...
    finalize_sweep(SweepFinalization {
        options: &options,
        publication,
        csv_manifest,
        effective_ir_hash,
        theta_sha256: theta_file.as_ref().map(|theta| theta.sha256.as_str()),
        draw_count,
        draw_workers,
        concurrency_mode,
        fused_capacity,
        ...
        concurrency_timing: concurrency_spike_timing,
        fused_timing: fused_spike_timing,
        setup_elapsed,
        sweep_started,
    })
}
