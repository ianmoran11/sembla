  ...
  "criteriaSatisfied": [
    {
      "id": "criterion-1",
      "status": "satisfied",
      "evidence": "Concrete implementation map cites IR/runtime symbols and exact line ranges; risks are severity-labelled."
    }
  ],
  "changedFiles": [
    ".pi-subagents/artifacts/outputs/9a9cb7f2/.pi-subagents/prd0008-runtime-analysis.md"
  ],
  "testsAddedOrUpdated": [],
  "commandsRun": [
    {
      "command": "targeted read/find/grep over PRD, sembla-ir, sembla-runtime source and tests",
      "result": "passed",
      "summary": "Mapped model validation, evaluator, executor, RNG, state hash, fixtures and exact symbols."
    },
    {
      "command": "git status --short && git diff --stat && git diff --cached --stat",
      "result": "passed",
      "summary": "No staged files; unrelated pre-existing .piprd and subagent artifacts are present in the worktree."
    }
  ],
  "validationOutput": [
    "Read-only source analysis completed; no source or test files modified.",
    "Git cached diff was empty."
  ],
  "residualRisks": [
    "CPU sequential reductions conflict with PRD two-pass wording.",
    "CUDA and host transcendental/compiler floating-point behavior may not be bit-identical.",
    "Views/summaries execution boundary is ambiguous under 'any validated model'."
  ],
  "noStagedFiles": true,
  "diffSummary": "Added only the requested analysis artifact; repository source unchanged.",
  "reviewFindings": [
    "high: crates/sembla-runtime/src/eval.rs:1481 - CPU aggregate sums are sequential, conflicting with PRD fixed-shape two-pass reductions.",
    "high: crates/sembla-runtime/src/rng.rs:114 - CPU f64::ln racing clocks require an explicit bit-equality strategy on CUDA.",
    "medium: crates/sembla-runtime/src/state.rs:489 - composed hashes include in-flight wire inputs and must not hash device state columns alone."
  ],
  "manualNotes": "Worktree was already dirty with .piprd and sibling subagent artifacts; none were staged or edited by this scout."
}
```
